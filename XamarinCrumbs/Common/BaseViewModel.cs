...
namespace XamarinCrumbs.Common
{
    public class BaseViewModel : INotifyPropertyChanged
    {
        ...
        protected async Task<bool> InvokeWithErrorHandling(Task task, Func<Exception, Task<bool>> errorHandler = null)
        {
            ...
            (object result, bool succeeded) r = await this.InvokeWithErrorHandling(innerTask, errorHandler)
                                                          .ConfigureAwait(false);

            return await Task.FromResult(r.succeeded);
        }

        protected async Task<(T result, bool succeeded)> InvokeWithErrorHandling<T>(
            Task<T> task, Func<Exception, Task<bool>> errorHandler = null)
        {
